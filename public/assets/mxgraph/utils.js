...
function Model__AccessPointCreate(nameContainer) {
  Graph__log("Model__AccessPointCreate")
  var nameContainer = Model__ACCESSPOINT_BASENAME + Model__currentAccessPointID;
  // Update elementID
  while (Model__AppScope.containerExists(nameContainer)) {
    Model__currentAccessPointID++;
    nameContainer = Model__ACCESSPOINT_BASENAME + Model__currentAccessPointID;
  }
  const theContainer = Model__AppScope.newAccessPoint(nameContainer);
  Model__currentAccessPointID++;
  return theContainer;
}
...
function addSidebarAccessPointIcon(graph, sidebar) {
  // Function that is executed when the image is dropped on
  // the graph. The cell argument points to the cell under
  // the mousepointer if there is one.
  Graph__log("addSidebarAccessPointIcon")
  var funct = function(graph, evt, cell, x, y) {
    var nameContainer = Model__ElementCreate();
    Graph__ElementCreate(graph, nameContainer, x, y);
  }

  const Graph__NetworkElementImage =  'assets/docker_image_icons/access_point.png';
  // Creates the image which is used as the sidebar icon (drag source)
  var img = document.createElement('img');
  img.setAttribute('src', Graph__NetworkElementImage);
  // img.style.width = '48px';
...
function mxAddAccessPointElement(graph, sidebar) {
  //addSidebarAccessPointIcon(graph, sidebar);
  _addSidebarElment(graph, sidebar, 'assets/docker_image_icons/access_point.png', "AccessPoint", Model__AccessPointCreate, Graph__AccessPointCreate);
...
function MX__Main(container, outline, toolbar, sidebar, status, appScope) {
  ...
  if (!mxClient.isBrowserSupported()) {
    ...
    mxAddAccessPointElement(graph, sidebar);
