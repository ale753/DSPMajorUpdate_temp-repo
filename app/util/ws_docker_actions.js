...
exports.composeUp = function composeUp(params, body, callback, notifyCallback) {
  ...
  async.waterfall([
    ...
    (theConfig, cb) => {
      ...
      //read the docker compose and check if there is a lab that requires privileged and host as parameters
      // privileged and host options required for access point containers, but they could be required for other type
      // of containers. These procedure should be changed if new containers with these options will be added
      const fs = require('fs');
      const yaml = require('js-yaml');
      const homedir = require('os').homedir();
      var data;
      try {
      let fileContents = fs.readFileSync(homedir + '/dsp/' + params.namerepo + '/' + params.namelab + '/docker-compose.yml', 'utf8');
      ...
      Object.keys(data["services"]).forEach(function(key) {
        ...
        if("privileged" in data["services"][key] && "network_mode" in data["services"][key] ){
          if(data["services"][key]["privileged"] == true && data["services"][key]["network_mode"] == "host"){
            radios = radios + 1;
          }
        }
    });
      ...
      //here should go params.radios but it is undefined?
        exec("modprobe mac80211_hwsim radios=" + radios, (error, stdout, stderr) => {
        if (error) {
...
exports.composeDown = function composeDown(params, body, callback, notifyCallback) {
  ...
  if(radios > 0) {
    ...
    exec("rmmod mac80211_hwsim", (error, stdout, stderr) => {
    if (error) {
        console.log(`error: ${error.message}`);
        return;
    }
